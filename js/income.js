...
const Income = {
      ...
      <form id="incomeForm" autocomplete="off" onsubmit="Income.save(event, ${isEdit ? `'${record.id}'` : 'null'})">
        ...
        <div style="display:flex; gap:12px; margin-bottom:12px">
          <div style="flex:1">
            ...
            <input type="number" class="form-input" name="amount" step="0.01" min="0" value="${isEdit ? record.amount : ''}" placeholder="0.00" style="font-size:1.4rem; font-weight:800; height:60px; padding:8px; color:var(--success)">
          </div>
        </div>
  ...
  save(e, existingId = null) {
    ...
    if (partyName && partyName !== 'General') {
      ...
      if (!exists) {
        DB.add(DB.COLLECTIONS.PARTIES, { name: partyName, type: 'customer', phone: '', notes: '' });
      }
    }
